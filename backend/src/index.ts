...
import { serveGatePage, verifyGatePin, requireGateCookie, GATE_PATH } from './middleware/masterGate.middleware'
...
// ── Master Gate ──────────────────────────────────────────────────────────────
// GET /{secret-path} → show PIN page
// POST /api/master-gate/verify → validate PIN → set cookie → redirect
app.get(`/${GATE_PATH}`, serveGatePage)
app.post('/api/master-gate/verify', verifyGatePin)
// GET /api/master-gate/check → return 200 if cookie valid (for frontend check)
app.get('/api/master-gate/check', (req, res) => {
  const cookieParser = require('cookie')
  const raw = req.headers.cookie || ''
  const cookies = Object.fromEntries(raw.split(';').map((c: string) => c.trim().split('=').map(decodeURIComponent)))
  res.json({ ok: true })  // cookie check done by requireGateCookie middleware
})
...
// Protect /master — require gate cookie
app.get('/master', requireGateCookie, (_req, res) => {
