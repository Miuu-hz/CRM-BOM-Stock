export function applySchema(db: any): void {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS user_approval_permissions (
      ...
      approval_limit REAL DEFAULT 0,         -- อนุมัติได้สูงสุดเท่าไหร่ (0 = ไม่จำกัดถ้า unlimited=1)
    ...
    CREATE TABLE IF NOT EXISTS pos_daily_sales (
      ...
      net_profit REAL DEFAULT 0,         -- กำไรขาดุทธิ
    ...
    CREATE TABLE IF NOT EXISTS subscription_plans (
      id TEXT PRIMARY KEY,
      code TEXT UNIQUE NOT NULL,        -- free, starter, business, enterprise
      name TEXT NOT NULL,
      description TEXT,
      price_monthly REAL DEFAULT 0,
      price_yearly REAL DEFAULT 0,
      max_users INTEGER,                -- NULL = ไม่จำกัด
      max_products INTEGER,             -- NULL = ไม่จำกัด
      max_pos_shifts INTEGER,           -- NULL = ไม่จำกัด
    ...
    CREATE TABLE IF NOT EXISTS tenant_subscriptions (
      ...
      status TEXT NOT NULL DEFAULT 'FREE',  -- FREE, ACTIVE, EXPIRED, CANCELLED
      billing_cycle TEXT,                   -- monthly, yearly (NULL = free)
      current_period_start TEXT,
      current_period_end TEXT,              -- NULL = ไม่หมดอายุ (free)
      notes TEXT,
      created_at TEXT DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT DEFAULT CURRENT_TIMESTAMP
    );
