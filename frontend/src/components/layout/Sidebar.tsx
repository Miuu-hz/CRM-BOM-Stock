...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    tKey: 'sidebar.stock',
    icon: Package,
    descriptionKey: 'sidebar.stockDesc',
    isParent: true,
    // กดที่เมนูแม่จะไปที่ subMenu[0] จึงต้องให้ /stock เป็นตัวแรกเสมอ
    subMenu: [
      { path: '/stock', tKey: 'sidebar.stockItems', icon: Package },
      { path: '/stock/subcontractors', tKey: 'sidebar.subcontractors', icon: Wrench },
    ],
  },
  ...
  { path: '/tax', tKey: 'sidebar.tax', icon: Percent, descriptionKey: 'sidebar.taxDesc' },
  {
    ...
    subMenu: [
      ...
      { path: '/accounting/reconcile', tKey: 'sidebar.clearingReconcile', icon: ClipboardCheck },
      { path: '/accounting/platform-settlement', tKey: 'sidebar.platformSettlement', icon: Globe },
