...
function CostSimulation() {
  ...
  // BOM 1 ชุดอาจผลิตได้มากกว่า 1 หน่วย (R2) — ตัวเลขในหน้านี้ทั้งหมด (originalTotal/simulatedTotal/
  // totalChange) คือ "ต่อ 1 ชุดผลิต" อยู่แล้ว (ไม่ได้คูณด้วยจำนวนที่จะผลิต) จึงต้องหารด้วย outputQty
  // ก่อนจะพูดถึง "ต่อหน่วย" จริง ๆ
  const outputQtyPerBatch = Number((selectedBom as any)?.outputQty ?? (selectedBom as any)?.output_qty) || 1
  const outputUnitOfSelected = (selectedBom as any)?.outputUnit || (selectedBom as any)?.output_unit || ''
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-1 min-w-[200px]">
            ...
            >
              ...
              {(boms || []).map((bom) => {
                const u = (bom as any).outputUnit || (bom as any).output_unit
                return (
                  <option key={bom.id} value={bom.id}>
                    {bom.product.name} ({bom.version}) - ฿{bom.totalCost.toLocaleString()}{u ? ` / ${unitLabel(u)}` : ''}
                  </option>
                )
              })}
            </select>
          ...
          {Math.abs(totalChangePercent) > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  {/* ตัวเลขนี้คือผลกระทบต่อ "1 ชุดผลิต" (เบิกวัตถุดิบ 1 รอบ) ซึ่งอาจได้ผลผลิตมากกว่า 1 หน่วย (R2)
                      เดิมป้ายบอกว่า "Per Unit" แต่ตอนนี้ไม่จริงแล้วถ้า BOM มี outputQty > 1 */}
                  <p className="text-[var(--fg-3)] mb-2">
                    {outputQtyPerBatch === 1
                      ? t('bom.simulation.perUnitImpact')
                      : t('bom.simulation.perBatchImpact', {
                          qty: outputQtyPerBatch.toLocaleString('th-TH'),
                          unit: outputUnitOfSelected ? unitLabel(outputUnitOfSelected) : '',
                        })}
                  </p>
                  <p
                ...
                <div>
                  <p className="text-[var(--fg-3)] mb-2">{t('bom.simulation.for100Units')}</p>
                  <p
                    className={`text-3xl font-bold ${
                      totalChange > 0 ? 'text-danger' : 'text-success'
                    }`}
                  >
                    {/* แก้จาก totalChange * 100 (= 100 ชุดผลิต) เป็นค่าต่อหน่วยจริงคูณ 100 (= 100 หน่วยผลผลิต) */}
                    {totalChange > 0 ? '+' : ''}฿{((totalChange / outputQtyPerBatch) * 100).toLocaleString()}
                  </p>
                </div>
              </div>
