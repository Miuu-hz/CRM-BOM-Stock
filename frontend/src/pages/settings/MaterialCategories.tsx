    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-100">หมวดหมู่วัตถุดิบ</h2>
            <p className="text-xs text-gray-400">จัดการหมวดหมู่และหน่วยเริ่มต้นสำหรับวัตถุดิบใน BOM</p>
          </div>
        </div>
        <button
          onClick={openCreate}
      ...
      {/* Info */}
      <div className="flex items-start gap-2 p-3 bg-cyber-primary/5 border border-cyber-primary/20 rounded-lg">
        <Info className="w-4 h-4 text-cyber-primary mt-0.5 flex-shrink-0" />
      ...
      <div className="relative">
        ...
        <input
          ...
          className="cyber-input w-full pl-9 pr-4"
        />
        {searchTerm && (
      ...
      <div className="cyber-card overflow-hidden">
        <div className="p-4 border-b border-cyber-border flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-gray-200">รายการหมวดหมู่</span>
          <span className="px-2 py-0.5 bg-cyber-primary/20 text-cyber-primary text-xs rounded-full">
        ...
        {loading ? (
          ...
          <div className="p-6 text-center">
            ...
            <p className="text-gray-500 text-sm">ยังไม่มีหมวดหมู่วัตถุดิบ</p>
            <p className="text-gray-600 text-xs mt-1">เพิ่มหมวดหมู่แรก เช่น วัตถุดิบทั่วไป, สารเคมี, อุปกรณ์</p>
          </div>
        ) : (
          <div className="divide-y divide-cyber-border/30">
            {filtered.map((c) => (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex items-center gap-3 px-4 py-3 hover:bg-cyber-card/30 transition-colors"
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 rounded-lg bg-cyber-primary/10 flex items-center justify-center flex-shrink-0">
            ...
            >
              ...
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-gray-400 mb-1.5">รหัส (Code) *</label>
                    <input
                      type="text"
                      value={code}
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1.5">หน่วยเริ่มต้น *</label>
                    <select
                      value={defaultUnit}
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1.5">ชื่อหมวดหมู่ *</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1.5">คำอธิบาย (ไม่บังคับ)</label>
                  <input
                    type="text"
                    value={description}
                ...
                <div className="flex gap-3 pt-1">
                  <button onClick={closeForm} className="flex-1 py-2.5 border border-cyber-border text-gray-300 hover:text-gray-100 rounded-lg text-sm transition-colors">
                    ยกเลิก
                  </button>
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex-1 py-2.5 bg-cyber-primary hover:bg-cyber-secondary disabled:opacity-50 text-black rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
                  >
